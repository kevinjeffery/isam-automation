...
class DB2Database:
  ...
  def update_db_cfg(self, name, value):
    ...
    if not self._created(): 
      self.module.fail_json(changed=False, msg="Database {0} does not exist.".format(self.dbname))
    cmd_connect = "db2 connect to {0}>/dev/null; ".format(self.dbname)
    cmd_terminate = "; db2 terminate>/dev/null"
    ...
    rc, stdout, stderr = self._db2_command(cmd_connect + cmd + cmd_terminate, cmd + " Failed")
...
#      options += " with '{0}'".format(db_comment)
  ...
  def create_db_tablespace(self, tablespace, type, bufferpool, pagesize):
    ...
    rc, stdout, stderr = self._db2_command(cmd_connect + cmd + options + cmd_terminate, cmd + " Failed")
  ...
  def _get_db_cfg_items(self):
    ...
    rc, stdout, stderr = self._db2_command(cmd_connect + cmd + cmd_terminate, "Error, could not get DB config")
    config_items = []
  ...
  def _get_db_bp_items(self):
    ...
    rc, stdout, stderr = self._db2_command(cmd_connect + cmd + cmd_terminate,"Error, could not get DB Bufferpools")
    config_items = []
    config_info=stdout.split('\n')
    for config_line in config_info:
      try:
  ...
  def _get_db_ts_items(self):
    ...
    rc, stdout, stderr = self._db2_command(cmd_connect + cmd + cmd_terminate,"Error, could not get DB Tablespaces")
    config_items = []
    config_item = None
    config_info=stdout.split('\n')
    for config_line in config_info:
